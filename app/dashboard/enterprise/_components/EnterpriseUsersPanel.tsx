                    ...
                    <div className="flex items-center gap-1 flex-wrap">
                      ...
                      <Popover open={popoverOpenFor === user.id} onOpenChange={async (open) => { if (open) await fetchTenantTags(); setPopoverOpenFor(open ? user.id : null); }}>
                        <PopoverTrigger asChild>
                          <button type="button" className="inline-flex items-center justify-center rounded border border-border h-5 w-5" title="Manage tags" onClick={(e) => e.stopPropagation()}><Plus className="h-3 w-3" /></button>
                        </PopoverTrigger>
